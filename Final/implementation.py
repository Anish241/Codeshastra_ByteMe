...
def main():
    ...
    while True:
        ...
        print(len(sequence))
        ret, frame = vid.read()
        hand_box,boundingRect = get_hand_image(frame)
        ...
        cv2.rectangle(frame, (int(boundingRect[0]), int(boundingRect[1])),
                          (int(boundingRect[0] + boundingRect[2]), int(boundingRect[1] + boundingRect[3])),
                          (0, 255, 0), 2)
